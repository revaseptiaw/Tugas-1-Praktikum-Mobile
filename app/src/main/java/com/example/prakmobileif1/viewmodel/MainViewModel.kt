package com.example.prakmobileif1.viewmodel
...
import com.example.prakmobileif1.data.network.RetrofitInstance
        ...
        viewModelScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    ...
                    _books.value = result
                    Log.d("SUCCESS_GET_ADD", "$result")
                } else {
                    Log.d("API_ERROR", "${response.code()} ${response.message()}")
                }
            } catch (e: Exception) {
